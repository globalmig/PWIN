...
const companyList = [
  {
    ...
    name: "CEO",
    link: "/company",
    image: "/images/company/mini (1).jpg",
  },
  {
    id: 2,
    name: "회사연혁",
    link: "/company#history",
    image: "/images/company/history02.jpg",
  },
  {
    id: 3,
    name: "인증현황",
    link: "/company#certificate",
    image: "/images/company/mini (4).jpg",
  },
  {
    id: 4,
    name: "오시는 길",
    link: "/company#location",
    image: "/images/company/mini (5).jpg",
  },
];
...
export default function CompanyList() {
  return (
    <div className="flex flex-col items-center justify-center w-full px-4 my-10  md:my-28 ">
          ...
          <li className="flex flex-col items-center flex-1  hover:flex-[2] transition-all duration-500 ease-in-out" key={item.id}>
            <Link href={item.link}>
              <div className="relative">
                ...
                <p className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white text-2xl lg:text-4xl font-bold [text-shadow:2px_2px_4px_rgba(0,0,0,0.8)]">{item.name}</p>
              </div>
            </Link>
          </li>
        ))}
      ...
      {/* 모바일 버전  */}
      <ul className="flex flex-col gap-4 w-full items-center justify-center text-xl font-medium py-0 md:py-20 md:hidden">
        {companyList.map((item) => (
